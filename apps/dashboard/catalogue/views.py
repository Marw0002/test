...
class ProductCreateUpdateView(generic.UpdateView):
    ...
    def get_page_title(self):
        if self.creating:
            if self.parent is None:
                return _('Create new inspiration')					
            else:
    ...
    def process_all_forms(self, form):
        ...
        if self.creating and form.is_valid():
            color_values=form.cleaned_data['color']
            theme_values=form.cleaned_data['theme']
            color_name=""
            theme_name=color_name			
            for color_value in color_values:
                color_name+=str(color_value.name)			
            for theme_value in theme_values:			
                theme_name+=str(theme_value.name)			
            				
            self.object = form.save()
            self.object.color_name=color_name
            self.object.theme_name=theme_name			
            self.object.save()
