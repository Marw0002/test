...
class FacetedSearchView(views.FacetedSearchView):
    ...
    def extra_context(self):
        ...
        extra['colors']=Color.objects.all()
        extra['themes']=Theme.objects.all()
...
class FacetedSearchVendorView(views.FacetedSearchView):
    """
    A modified version of Haystack's FacetedSearchView
    ...
    # Haystack uses a different class attribute to CBVs
    template = "search/results_vendor.html"
    search_signal = signals.user_search

    def __call__(self, request):
        response = super(FacetedSearchVendorView, self).__call__(request)
    ...
    # have to do less work in the template.
    def extra_context(self):
        extra = super(FacetedSearchVendorView, self).extra_context()
        ...
        # form.
        extra['selected_facets'] = self.request.GET.getlist('selected_facets')
        extra['search_form2'] = self.form
        return extra

    def get_results(self):
        # We're only interested in products (there might be other content types
        # in the Solr index).
        return super(FacetedSearchVendorView, self).get_results().models(Company)