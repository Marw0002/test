...
class SearchForm(FacetedSearchForm):
    ...
    q = forms.CharField(
        ...
        widget=SearchInput({
            "placeholder": _('Search for Inspiration'),
            "tabindex": "1",
            "class": "form-control"
        }))
...
class SearchForm2(FacetedSearchForm):
    """
    In Haystack, the search form is used for interpreting
    and sub-filtering the SQS.
    """
    ...
    q = forms.CharField(
        required=False, label=_('Search'),
        widget=SearchInput({
            "placeholder": _('Search for Vendor'),
            "tabindex": "1",
            "class": "form-control"
        }))
    ...
    TOP_RATED = "rating"
    NEWEST = "newest"
    COMPANY_NAME_A_TO_Z = "company-asc"
    COMPANY_NAME_Z_TO_A = "company-desc"

    SORT_BY_CHOICES = [
        (RELEVANCY, _("Relevancy")),
        (TOP_RATED, _("Customer rating")),
        (NEWEST, _("Newest")),
        (COMPANY_NAME_A_TO_Z, _("Company name A to Z")),
        (COMPANY_NAME_Z_TO_A, _("Company name Z to A")),
    ]
    ...
    SORT_BY_MAP = {
        TOP_RATED: '-rating',
        NEWEST: '-date_created',
        COMPANY_NAME_A_TO_Z: 'company_name_s',
        COMPANY_NAME_Z_TO_A: '-company_name_s',
    }
    # Non Solr backends don't support dynamic fields so we just sort on title
    if not is_solr_supported():
        SORT_BY_MAP[COMPANY_NAME_A_TO_Z] = 'company_name_exact'
        SORT_BY_MAP[COMPANY_NAME_Z_TO_A] = '-company_name_exact'
